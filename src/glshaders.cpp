    ...
    : m_compileError(false), m_failed(false)
{
    ...
    if (file.open(QIODevice::ReadOnly)) {
        ...
        qCritical("Fail to open Shader File...");
        m_compileError = true;
    }
}
...
bool GLProgram::hasParameter(const QString& name)
{
    ...
    if (!failed()) {
        QByteArray asciiName = name.toStdString().c_str();
...
void GLProgram::setMatrix(const QString& name, const float mat[16])
{
    ...
    if (!failed()) {
        ...
        int loc = glGetUniformLocationARB(m_program, asciiName.data());
        //qDebug() << "Location of" << name << "is" << loc;
        glUniformMatrix4fvARB(loc, 1, GL_FALSE, mat);
    }
}
